  ...
  template: `
    @if (region().hasKB) {
      <a
        ...
        [attr.aria-label]="
          region().name +
          (stats().poiCount !== undefined ? ' — ' + stats().poiCount + ' luoghi' : '')
        "
      >
        @if (!imageError() && imageUrl()) {
          ...
          />
          <div
            class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent rounded-2xl"
          ></div>
        }
        ...
        <div class="relative z-10 flex flex-col gap-1 min-w-0">
          <span
            class="font-display font-semibold leading-snug line-clamp-2 break-words"
            [class]="imageError() ? 'text-text' : 'text-white'"
          >
            {{ region().name }}
          </span>
          ...
          @if (stats().poiCount !== undefined) {
            <span
              class="text-xs mt-1"
              [class]="imageError() ? 'text-text-secondary' : 'text-white/80'"
            >
      ...
      >
        ...
        <div class="relative z-10 flex items-start justify-between gap-2">
          ...
          ></svg>
          <span
            class="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-surface-inset text-text-tertiary whitespace-nowrap"
          >
            In arrivo
          </span>
        </div>
